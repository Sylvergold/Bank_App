...
exports.elecTrans = async (req, res)=>{
  try {
    ...
    const electricity = await electricityModel.findById(elecId)
    if (!user) {
    ...
    const comparetransacPin = bcryptjs.compareSync(transacPin, user.transactionPin);
    ...
    const electricityHist = await new transactionHistoryModel({
